...
Here we use R to take 5000 samples of size 50 from the population, calculate the mean of each sample, and store each result in a vector called sample means50. On the next page, we’ll review how this set of code works.
...
for (i in 1:5000) {
	samp <- sample(area, 50)
	sample_means50[i] <- mean(samp)
	# print(i) # omitted to save space
}
```

Let’s consider this code line by line to figure out what it does. In the first line we initialized a vector. In this case, we created a vector of 5000 zeros called sample means50. This vector will will store values generated within the for loop.
...
The body of the for loop is the part inside the curly braces, and this set of code is run for each value of i. Here, on every loop, we take a random sample of size 50 from area, take its mean, and store it as the ith element of sample means50.
...
# This will return the plottting window to normal
par(mfrow = c(1, 1))
```
